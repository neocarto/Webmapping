...
Il existe d'autres API de géocodage.
...
Il permet de calculer des trajets, des distances et des durées de parcours pour différents modes de transport (voiture, vélo, piéton, etc.), un peu comme Google Maps, mais en local ou sur un serveur privé. OSRM ets dosponible à l'adresse suivante : [https://project-osrm.org](https://project-osrm.org/).

La documentation est accessible ici : [https://project-osrm.org/docs/v5.24.0/api/#](https://project-osrm.org/docs/v5.24.0/api/#)
...
- `route` correspond au service
- `car` correspond au moyen de transport. On peut aussi utiliser `bike` et `foot`.
...
Ce qui permet de visualiser le trajeyt sur une carte. 
...
::: {.callout-note title="Exercice"}
Calculer le temps de parcours à velo entre le batiment Olympe de Gouges et l'institut de geographie. Et affivchez la carte. 
...
async function main() {
  // Calcul du tracé de la route avec OSRM
  const url = `https://router.project-osrm.org/route/v1/driving/${bourgdoisans[1]},${bourgdoisans[0]};${alpeshuez[1]},${alpeshuez[0]}?geometries=geojson&overview=full`;
  const response = await fetch(url);
  const data = await response.json();
  document.getElementById("temps").textContent =
    "Temps de parcours" + data.routes[0].duration + " minutes";

  // Récupération du tracé et des coordonnées
  const trace = data.routes[0].geometry;
  const coords = trace.coordinates.filter((_, i) => i % 3 === 0);
  const lon = coords.map((d) => d[0]).join("|");
  const lat = coords.map((d) => d[1]).join("|");

  // Calcul du profil altimétrique
  const req = `https://data.geopf.fr/altimetrie/1.0/calcul/alti/rest/elevation.json?lon=${lon}&lat=${lat}&resource=ign_rge_alti_wld&delimiter=|&indent=false&measures=false&zonly=false`;
  const response2 = await fetch(req);
  const data2 = await response2.json();

  // Création du graphique
  const altitudes = data2.elevations.map((d, i) => ({ x: i, y: d.z }));
  const chart = Plot.plot({
    marks: [
      Plot.line(altitudes, { x: "x", y: "y" }), // ligne
      Plot.dot(altitudes, { x: "x", y: "y" }), // points
  ...
  // Récupération de la météo au sommet
  const url_meteo = `https://api.open-meteo.com/v1/forecast?latitude=${alpeshuez[0]}&longitude=${alpeshuez[1]}&&current_weather=true&timezone=Europe%2FLondon`;
  const response3 = await fetch(url_meteo);
  const data3 = await response3.json();
  document.getElementById("meteo").textContent =
    "Il fait " + data3.current_weather.temperature + " °C au sommet";
}
// Appel de la fonction
main();
