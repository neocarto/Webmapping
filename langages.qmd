...
Pour écrire du code, on utilise génralement une IDE (interface de développement). Il en existe de nombreux sur la marché et un certains nombre d'entre eux sont gratuits. Mais on peut utiliser un simple éditeur de texte type **notepad** ou **notepad++**. Ensuite, il suffit d'ouvrir les fichiers avec votre navigateur web préféré. 
...
<form>
  ...
  <select id="style" name="style">
    ...
    <option value="rap">Rap</option></select
  ><br />
...
En savoir plus sur le SVG : [observablehq.com/@neocartocnrs/le-format-svg](https://observablehq.com/@neocartocnrs/le-format-svg?collection=@neocartocnrs/observable-cest-quoi)
...
ECMAScript 2025 : les nouveautés JavaScript à connaître : [voir](https://mink-agency.com/ecmascript-2025-les-nouveautes-javascript-a-connaitre/#:~:text=ECMAScript%20%C3%A9volue%2C%20et%20avec%20la,plus%20intelligents%2C%20expressions%20r%C3%A9guli%C3%A8res%20am%C3%A9lior%C3%A9es%E2%80%A6)

### Une grande communauté

C'est un langage ancien qui dispose d'une très grande communauté.

<img src="img/js_chart.png" class="wide"/>
...
Voici à quoi ca ressemble. En fait, il s'agit d'un Array dont chaque élément est un objet. C'est la façon qu'on a en JavaScrit de formaliser un tableau. 
...
const cycles = books.map(d => d.cycle)
const cyclesUniques = new Set(cycles)
console.log(cyclesUniques)
```

Exemple de code classique (`Set` et `Array.from`)
...
fetch("https://raw.githubusercontent.com/neocarto/Webmapping/refs/heads/main/data/cities.json")
  ...
  .then(data => {
    ...
    // Parmi ces villes de plus de 500 000 habitants, combien y en a t-il dans l'hémisphère Sud ? 

    // Combien y en a t-il de villes de plus de 500 000 habitants en France ?

    // Affichez un Array avec la liste des villes
...
Il y a plusieurs façons d'importer une bibliothèque. La documentation de chaque bibliothèque explique comment il faut procéder et dans quel contexte. Voici un exemple. 
...
```
Pour utiliser la syntaxe avec `import`, le navigateur doit charger le script avec `type="module"`

Nous verrons la bibliothèques `geotoolbox` plus tard.


# La structure d'un document web

Dans `jsfiddle`, le code est réparti dans 3 cadrants (html, css et js). Mais lorsque vous téléchargez le fiddle, l'ensemble du code est regroupé dans un seul ficher html qui est organisé comme ceci.
